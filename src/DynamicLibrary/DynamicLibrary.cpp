...
namespace RayTracer
{
    ...
    void DynamicLibrary::changeLibrary(std::string libraryName)
    {
        dlclose(this->library);
    ...
    std::string DynamicLibrary::getName(std::string functionName)
    {
        if (_name == "") {
    ...
    std::unique_ptr<Module> DynamicLibrary::getModule(std::string functionName, Args&&... args)
    {
